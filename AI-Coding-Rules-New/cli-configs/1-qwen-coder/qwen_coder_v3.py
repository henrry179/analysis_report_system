...
def download_model(model_name="Qwen/Qwen3-Coder-30B-A3B-Instruct"):
    """下载Qwen3 Coder模型"""
    print(f"📥 下载模型: {model_name}")
    print("⚠️  注意: 模型文件较大(约60GB)，下载需要时间")
...
def create_web_interface(tokenizer, model):
    ...
    try:
        ...
        def generate_code(prompt, max_length=2048, temperature=0.7):
            """生成代码"""
            try:
                messages = [
        ...
        # 创建界面
        with gr.Blocks(title="Qwen3 Coder - 代码生成助手") as interface:
            gr.Markdown("# 🤖 Qwen3 Coder - 通义千问代码生成助手")
            gr.Markdown("支持代码生成、解释、优化等多种功能，支持358种编程语言")
            ...
            with gr.Tab("代码生成"):
                ...
                max_length_slider = gr.Slider(
                    minimum=100, maximum=65536, value=2048, step=100,
                    label="最大生成长度 (tokens)"
                )
            ...
            with gr.Tab("代码优化"):
                ...
                language_selector = gr.Dropdown(
                    ["Python", "JavaScript", "Java", "C++", "Go", "Rust", "TypeScript", "C#", "PHP"],
                    label="编程语言",
                    value="Python"
                )
...
st.set_page_config(page_title="Qwen3 Coder", page_icon="🤖")

st.title("🤖 Qwen3 Coder - 代码生成助手")
st.markdown("基于通义千问Qwen3-Coder模型，支持358种编程语言")

# 侧边栏配置
with st.sidebar:
    st.header("⚙️ 配置")
    max_length = st.slider("最大生成长度", 100, 65536, 2048)
...
with tab3:
    ...
    language = st.selectbox("编程语言", ["Python", "JavaScript", "Java", "C++", "Go", "Rust", "TypeScript", "C#", "PHP"])
...
'''
    ...
    with open("qwen_coder_app_v3.py", "w", encoding="utf-8") as f:
        f.write(streamlit_code)

    print("✅ Streamlit应用创建完成: qwen_coder_app_v3.py")
    print("运行命令: streamlit run qwen_coder_app_v3.py")
...
def main():
    parser = argparse.ArgumentParser(description="Qwen3 Coder - 通义千问代码生成模型")
