 ...
 * 文件名: 10-循环日销量表.sql
 * 功能: 循环门店销售分析
 * 描述: 本脚本通过循环处理每日销售数据，统计门店销售与库存情况，支持按门店、商品、备注等多维度分析
 */
...
DECLARE @DDP_PERIOD VARCHAR(6); -- 大单品期间代码
...
SET @DDP_PERIOD = '202401';                                                -- 大单品期间代码，如202401表示2024年1月
...
SELECT 
    ...
    wrh,                          -- 仓位
    qty AS 库存数量,              -- 库存数量
    qty * ISNULL(whsprc, 0) AS 库存金额  -- 计算库存金额（数量*单价）
INTO #stk 
...
INTO #store2 
FROM #stores a
CROSS JOIN (SELECT DISTINCT remark FROM #ddplist) b  -- 获取所有唯一的备注值
 ...
 * 7. 添加了更多调试信息输出，便于跟踪循环过程
 * 8. 优化了查询排序逻辑，提高数据可读性
 * 9. 添加了临时表清理代码，优化资源使用
 * 10. 优化了笛卡尔积查询，确保只包含有效的备注值
 * 11. 增加了对零动销、无库存等异常情况的分析维度
 */