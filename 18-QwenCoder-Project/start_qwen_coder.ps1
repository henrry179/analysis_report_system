...
Write-Host "🚀 Qwen Coder Windows 启动器" -ForegroundColor Green
...
switch ($Mode) {
    "web" {
        ...
        python qwen_coder.py --mode web --port $Port --model $Model
    }

    "streamlit" {
        Write-Host "📱 创建Streamlit应用..." -ForegroundColor Green
        python qwen_coder.py --mode streamlit

        Write-Host "" -ForegroundColor Yellow
        Write-Host "🚀 启动Streamlit应用:" -ForegroundColor Cyan
        Write-Host "streamlit run qwen_coder_app.py" -ForegroundColor White
        Write-Host "" -ForegroundColor Yellow
...
Write-Host "" -ForegroundColor Yellow
Write-Host "👋 Qwen Coder 已停止运行" -ForegroundColor Cyan
